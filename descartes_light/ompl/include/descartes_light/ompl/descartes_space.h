...
public:
   ...
   * difference). If they are 1 rung apart it will evaluate the cost of the edge. The rung it is going towards will then
   * have the cost of that vertex added unless it is going to the goal vertex which has a cost of 0. */
   ...
   * the side relative to the from state. This adjacent rung is then sequentially searched starting at the idx provided
   * by the to state, a check if performed to ensure no sampling is attempted outside of the valid range for this
   * searched rung. Once a valid edge + state with a cost less than the max_dist_ parameter is found, this state is
   * returned. */
