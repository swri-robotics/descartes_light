...
void descartes_light::DescartesStateSampler<FloatType>::sampleUniformNear(ompl::base::State *state, const ompl::base::State *near, const double /*distance*/)
{
  ...
  // Sample up if either it was randomly selected to sample up OR we are forced to if it is sampling near the first rung AND ensure we aren't sampling on the last rung
...
double descartes_light::DescartesStateSpace<FloatType>::distance(const ompl::base::State *state1, const ompl::base::State *state2) const
{
  ...
  // If they are on the same rung give a cost of 1 rung_to_rung_dist_, this should be seen as further away than points on adjacent
  // rungs, but closer than points separated by 1 rung in between
  if (rung_diff == 0)
  ...
  if (rung_diff == 1)
  {
    // If the min rung is the zeroth rung or the last added rung, then the cost should be 0. These states were added to give
    // a valid start and goal for the OMPL problem and should not be accounted for in the cost.
    if (min_rung == 0 || max_rung == ladder_rungs_.size() - 1)
      dist = 0;
    else
    {
      // Perform edge evaluation. Use the min_rung - 1 edge evaluator because there was an extra rung added to the graph to
      // provide a valid start state for OMPL which is not accounted for in the edge evaluator.
  ...
  // Ensure that this isn't going to the last rung artificially added, then add the cost of the higher rung in the connection
  ...
  // If a cost of zero was found, add the distance epsilon because OMPL requires that the distance between any 2 distinct states is >0
  if (dist == 0)
...
void descartes_light::DescartesStateSpace<FloatType>::interpolate(const ompl::base::State *from, const ompl::base::State *to, const double t, ompl::base::State *state) const
{
  ...
  else
  {
    ...
    // If initial rung difference is one, then return the from state because interpolation here can cause an endless loop in RRTConnect
    if (abs(rung_diff) == 1)
    {
    ...
    else
    {
      ...
      // Create new vertex if a valid connection was found, otherwise return from state because no interpolation was possible
