...
struct early_terminator_examine : public boost::base_visitor<early_terminator_examine<FloatType>>
{
  ...
  early_terminator_examine(long last_rung_idx) : last_rung_idx_(last_rung_idx) {}
...
/**
 * @brief Event visitor that terminates the search when a vertex in the last rung of the graph is discovered
 * @details Throws the vertex descriptor that is the termination of the path once a vertex in the last rung of
 * the graph is encountered
 */
template <typename FloatType>
struct early_terminator_discover : public boost::base_visitor<early_terminator_discover<FloatType>>
{
  /** @brief Event filter typedef defining the events for which this visitor can be used */
  typedef boost::on_discover_vertex event_filter;

  early_terminator_discover(long last_rung_idx) : last_rung_idx_(last_rung_idx) {}
...
/**
 * @brief Event visitor that adds all edges to each vertex dynamically as the vertex is discovered during the graph search
 */
template <typename FloatType>
struct add_all_edges_dynamically : public boost::base_visitor<add_all_edges_dynamically<FloatType>>
{
  /** @brief Event filter typedef defining the events for which this visitor can be used */
  typedef boost::on_discover_vertex event_filter;
