...
SearchResult<FloatType> BGLDijkstraSVDESolver<FloatType>::search()
{
  ...
  auto visitor = boost::make_dijkstra_visitor(add_all_edges_dynamically<FloatType>(edge_eval_, ladder_rungs_));
...
SearchResult<FloatType> BGLEfficientDijkstraSVDESolver<FloatType>::search()
{
  ...
  const long last_rung_idx = static_cast<long>(ladder_rungs_.size() - 1);
  auto visitor = boost::make_dijkstra_visitor(std::make_pair(
      early_terminator<FloatType>(last_rung_idx), add_all_edges_dynamically<FloatType>(edge_eval_, ladder_rungs_)));
