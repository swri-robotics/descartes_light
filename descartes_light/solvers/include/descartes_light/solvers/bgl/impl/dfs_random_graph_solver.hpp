 ...
 */
#ifndef DESCARTES_LIGHT_DFS_RANDOM_SOLVER_HPP
#define DESCARTES_LIGHT_DFS_RANDOM_SOLVER_HPP
...
class RandomVisitor : public boost::default_dijkstra_visitor
{
  public:
  RandomVisitor(std::vector<typename EdgeEvaluator<FloatType>::ConstPtr>& edge_eval,
  ...
  // using examine_vertex st. edges may be added on each traversal of the graph
  void examine_vertex(VertexDesc<FloatType> u, BGLGraph<FloatType> g)
  {
    if (g[u].rung_idx >= 0)
    {
      ...
      {
        //create a shuffled version of the next ladder rung
        std::vector<std::size_t> ti(ladder_rungs_[next_rung].size());
        std::iota(ti.begin(), ti.end(), 0);
        std::shuffle(ti.begin(), ti.end(), std::mt19937{std::random_device{}()});

        std::size_t attempt = 0;
        bool edge_added = false;
        while (!edge_added)
        {
          std::size_t to_indx = ti[attempt];
          ...
          {
            edge_added = true;
            FloatType cost = results.second + to_sample.sample.cost;
          ...
          {
            if (attempt != ladder_rungs_[next_rung].size())
            {
               ++attempt;
            }
            else
...
template <typename FloatType>
SearchResult<FloatType> DFSRandomLadderGraphSolver<FloatType>::search()
{
  ...
  descartes_light::RandomVisitor<FloatType> visitor(edge_eval_, ladder_rungs_, graph_);
...
#endif //DESCARTES_LIGHT_DFS_RANDOM_SOLVER_HPP
